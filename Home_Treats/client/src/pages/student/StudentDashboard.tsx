...
import { feesService, complaintService } from '../../services';
    ...
    const fetchData = async () => {
      ...
      try {
        const [feesRes, complaintsRes] = await Promise.all([
          feesService.getByStudent(user.studentId),
          complaintService.getByStudent(user.studentId),
        ]);
                  ...
                  recentNotifications.map((notification, idx) => (
                    <div
                      ...
                      className={`flex items-start space-x-3 p-3 rounded-lg ${
                        notification.color === 'green' ? 'bg-green-50' : 'bg-orange-50'
                      }`}
                    >
                      <div
                        className={`w-2 h-2 rounded-full mt-2 ${
                          notification.color === 'green' ? 'bg-green-600' : 'bg-orange-600'
                        }`}
                      ></div>
                      <div>
