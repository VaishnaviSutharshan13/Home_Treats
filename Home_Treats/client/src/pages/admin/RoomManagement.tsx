    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed top-4 right-4 z-[100] flex flex-col gap-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`px-5 py-3 rounded-xl shadow-lg text-white text-sm font-medium transition-all duration-300 flex items-center gap-2 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <Link to="/admin/dashboard" className="flex items-center text-gray-500 hover:text-blue-600 transition-colors">
                <FaHome className="w-5 h-5 mr-1.5" />
            ...
            <button
              ...
              className="bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 transition-all duration-200 flex items-center font-medium shadow-sm hover:shadow-md"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Rooms', count: rooms.length, icon: <FaBed className="text-xl" />, color: 'bg-blue-50 text-blue-700', accent: 'border-blue-200' },
            { label: 'Available', count: rooms.filter((r) => r.status === 'Available').length, icon: <FaCheck className="text-xl" />, color: 'bg-emerald-50 text-emerald-700', accent: 'border-emerald-200' },
            ...
            <div key={stat.label} className={`bg-white rounded-xl border ${stat.accent} p-4 flex items-center gap-4 shadow-sm`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {!loading && error && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <button onClick={fetchRooms} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors">
              Retry
            </button>
          </div>
        )}
        ...
        {!loading && !error && filteredRooms.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-gray-400">
            ...
            {!searchTerm && filterStatus === 'All' && (
              <button onClick={handleAddRoom} className="mt-4 text-blue-600 font-medium hover:underline">
                Add your first room
              </button>
            )}
          </div>
            ...
            {filteredRooms.map((room) => (
              ...
              >
                ...
                <div className="relative h-44 overflow-hidden">
                  ...
                  <span className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-lg text-xs font-bold text-gray-900 shadow-sm">
                ...
                <div className="p-4">
                  ...
                  {room.facilities && room.facilities.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {room.facilities.slice(0, 4).map((f) => (
                        <span key={f} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 text-xs font-medium">
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 text-sm font-medium text-emerald-600 bg-emerald-50 hover:bg-emerald-100 py-2 px-3 rounded-lg transition-colors"
                      title="View Room"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 py-2 rounded-lg transition-colors"
                      title="Edit Room"
                    >
                    ...
                    {room.occupied < room.capacity && room.status !== 'Maintenance' && (
                      <button
                        ...
                        className="flex items-center justify-center text-emerald-600 bg-emerald-50 hover:bg-emerald-100 p-2 rounded-lg transition-colors"
                    ...
                    {room.occupied > 0 && (
                      <button
                        ...
                        className="flex items-center justify-center text-amber-600 bg-amber-50 hover:bg-amber-100 p-2 rounded-lg transition-colors"
                        title="Vacate Room"
                      >
                        <FaSignOutAlt />
                      </button>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="px-6 py-5 max-h-[70vh] overflow-y-auto space-y-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className="w-28 h-28 rounded-xl border-2 border-dashed border-gray-300 hover:border-blue-400 flex items-center justify-center overflow-hidden cursor-pointer transition-colors relative group"
                      ...
                      <div className="text-xs text-gray-500">
                        ...
                        {formData.image && (
                          <p className="mt-1 text-blue-600 font-medium">{formData.image.name}</p>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-white"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Beds (Capacity) *</label>
                      <input
                        type="number"
                        min={1}
                        max={10}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                          <span className="text-gray-400 text-sm font-medium">LKR</span>
                        </div>
                        <input
                          type="number"
                          min={4000}
                          ...
                          className="w-full border border-gray-300 rounded-lg pl-12 pr-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                        />
                      </div>
                    </div>
                    {editingRoom && (
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all resize-none"
                ...
                <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm"
                  >
                    {submitting ? (
                      <>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
              >
                {allocating ? (
                  <>
                    <FaSpinner className="animate-spin" />
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-gray-900">{viewingRoom.roomNumber}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Block / Floor</p>
                  <p className="text-gray-900">{viewingRoom.block} / {viewingRoom.floor}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Type</p>
                  <p className="text-gray-900">{viewingRoom.type}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Price</p>
                  <p className="text-gray-900 font-semibold">{formatLKR(viewingRoom.price)}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Capacity</p>
                  <p className="text-gray-900">{viewingRoom.occupied} / {viewingRoom.capacity}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Location</p>
                  <p className="text-gray-900">{viewingRoom.location || 'Jaffna, Sri Lanka'}</p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {viewingRoom.facilities.map((f) => (
                      <span key={f} className="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-full border border-blue-200">
                        {FACILITY_ICONS[f] || '•'} {f}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t">
                <button onClick={() => setViewingRoom(null)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Close</button>
                <button onClick={() => { handleEditRoom(viewingRoom); setViewingRoom(null); }} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center">
