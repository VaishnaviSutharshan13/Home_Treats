...
const FacilityItem: React.FC<FacilityItemProps> = ({ icon, title, desc }) => (
  <div className="flex flex-col items-center text-center bg-white rounded-xl shadow p-6 border border-primary/15 hover:shadow-lg transition-all duration-200">
    {icon}
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-8 text-center">
        <h1 className="text-4xl md:text-5xl font-black text-primary mb-3 tracking-tight">Find Your Perfect Student Room</h1>
        <p className="text-gray-500 text-lg font-medium max-w-2xl mx-auto">Browse and select from our available hostel rooms.</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        <form className="flex flex-wrap gap-4 items-end justify-center bg-surface-active border border-primary/15 rounded-2xl p-6 shadow-sm">
          <div>
            <label className="block text-sm font-semibold text-primary mb-1">Room Type</label>
            <select name="type" value={filters.type} onChange={handleFilterChange} className="rounded-xl border-gray-200 focus:ring-primary focus:border-primary text-sm px-4 py-2 w-40">
          ...
          <div>
            <label className="block text-sm font-semibold text-primary mb-1">Price Range</label>
            <select name="price" value={filters.price} onChange={handleFilterChange} className="rounded-xl border-gray-200 focus:ring-primary focus:border-primary text-sm px-4 py-2 w-40">
              <option value="">All</option>
          ...
          <div>
            <label className="block text-sm font-semibold text-primary mb-1">Availability</label>
            <select name="availability" value={filters.availability} onChange={handleFilterChange} className="rounded-xl border-gray-200 focus:ring-primary focus:border-primary text-sm px-4 py-2 w-40">
              <option value="">All</option>
          ...
          <button type="button" className="inline-flex items-center gap-2 px-6 py-2.5 bg-primary text-white font-bold rounded-xl text-sm mt-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {loading ? (
          <div className="py-16 text-center text-gray-500">Loading rooms...</div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredRooms.slice(0, visibleCount).map((room) => {
                ...
                return (
                  <div key={room._id} className="bg-white rounded-2xl border border-primary/15 shadow-lg group flex flex-col overflow-hidden relative hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="flex-1 flex flex-col p-6">
                      ...
                      <p className="text-gray-500 text-sm mb-4 flex-1">{room.description || 'Comfortable student accommodation with essential amenities.'}</p>
                      <div className="flex gap-3 mt-auto">
                        <button onClick={() => navigate(`/floor/${floorId}`)} className="flex-1 px-4 py-2 bg-primary hover:bg-primary-hover text-white font-bold rounded-xl text-sm">View Details</button>
                        <button onClick={() => navigate(`/floor/${floorId}/rooms`)} className="flex-1 px-4 py-2 bg-white border border-primary text-primary font-bold rounded-xl hover:bg-surface-active text-sm">Book Now</button>
                      </div>
                    </div>
                  </div>
                );
