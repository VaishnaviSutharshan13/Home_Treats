...
const isOwnerComplaint = async (complaint: any, userId: string) => {
  ...
  return (
    complaint.student === user.studentId || complaint.student === user.name
  );
};
...
const sanitizeUpdatePayload = (body: any) => {
  ...
  if (typeof body.resolutionNotes === "string")
    next.resolutionNotes = body.resolutionNotes.trim();
  if (typeof body.rejectionReason === "string")
    next.rejectionReason = body.rejectionReason.trim();
  if (body.estimatedResolution)
    next.estimatedResolution = body.estimatedResolution;
...
const isValidStatusTransition = (currentStatus: string, nextStatus: string) => {
  ...
  if (
    !ADMIN_ALLOWED_STATUSES.includes(currentStatus) ||
    !ADMIN_ALLOWED_STATUSES.includes(nextStatus)
  )
    return false;
  if (currentStatus === "Pending")
    return nextStatus === "In Progress" || nextStatus === "Rejected";
...
export const getCurrentUserComplaints = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
...
export const createComplaint = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user.role === "admin") {
      return res
        .status(403)
        .json({ success: false, message: "Admins cannot create complaints" });
    }
    ...
    const user = await User.findById(req.user.id).select(
      "name studentId room roomNumber",
    );
    if (!user) {
      return res
        .status(404)
    ...
    const studentValue = isStudent
      ? user.studentId || user.name || String(user._id)
      : payload.student;
    const roomValue = isStudent
      ? user.room || user.roomNumber || payload.room || "Unassigned"
      : payload.room || "Unassigned";

    if (!studentValue) {
      return res
        .status(400)
        .json({ success: false, message: "Student identity is required" });
    }
    ...
    const studentUser = await User.findOne({
      role: "student",
      studentId: savedComplaint.student,
    }).select("_id name");
  ...
  try {
    ...
    if (req.user.role === "student") {
      ...
      if (
        ...
        !ADMIN_ALLOWED_STATUSES.includes(updatePayload.status)
      ) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid status for admin update" });
      }

      if (
        updatePayload.status &&
        !isValidStatusTransition(complaint.status, updatePayload.status)
      ) {
        return res.status(400).json({
          success: false,
      ...
      if (
        ...
        !String(updatePayload.assignedTo || complaint.assignedTo || "").trim()
      ) {
        return res.status(400).json({
          success: false,
          message:
            "Assigned to is required when moving complaint to In Progress",
        });
      }
      ...
      if (
        ...
        !String(
          updatePayload.rejectionReason || complaint.rejectionReason || "",
        ).trim()
      ) {
        return res.status(400).json({
          success: false,
          message: "Rejection reason is required when rejecting complaint",
        });
      }
    ...
    if (
      ...
      !ALLOWED_CATEGORIES.includes(updatePayload.category)
    ) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid category" });
    ...
    if (
      ...
      !ALLOWED_PRIORITIES.includes(updatePayload.priority)
    ) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid priority" });
...
export const assignComplaint = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!complaint.assignedTo) {
      return res
        .status(400)
        .json({ success: false, message: "Assigned to is required" });
    }

    if (req.body.estimatedResolution)
      complaint.estimatedResolution = req.body.estimatedResolution;
  ...
  try {
    ...
    if (req.user) {
      await logAdminAction(
        req.user.email,
        String(req.user.id),
        `${complaint.status === "Resolved" ? "Resolved" : "Rejected"} a complaint`,
        "complaint",
        String(req.params.id),
        complaint.title,
      );
